-- Ecstays UI – Blue Dark + Login Gate (pixel-perfect to reference), smooth drag + inertia
...
local function tplay(o,t,props,style,dir) return TweenS:Create(o, TweenInfo.new(t, style or Enum.EasingStyle.Sine, dir or Enum.EasingDirection.Out), props):Play() end
local function rc(p,r) local u=Instance.new("UICorner");u.CornerRadius=UDim.new(0,r or 12);u.Parent=p;return u end
local function st(p,th,co,tr) local s=Instance.new("UIStroke");s.Thickness=th or 1;s.Color=co or Color3.fromRGB(70,95,140);s.Transparency=tr or .45;s.Parent=p;return s end
local function clamp01(x) return x<0 and 0 or (x>1 and 1 or x) end

-- theme (no purple)
local Theme = {
  Primary    = Color3.fromRGB(18,20,26),  -- titlebar
  Secondary  = Color3.fromRGB(22,25,32),  -- window
  Panel      = Color3.fromRGB(28,32,40),  -- panels/rows
  Interact   = Color3.fromRGB(36,41,50),  -- inputs/hover

  Title      = Color3.fromRGB(236,241,255),
  Text       = Color3.fromRGB(210,222,244),
  Muted      = Color3.fromRGB(164,178,206),

  Accent     = Color3.fromRGB(88,165,255), -- blue
...
local Ecstays = {}
Ecstays._VERSION = "5.0-gate-blue"
...
function Ecstays:CreateWindow(opt)
  ...
  local LOGIN = opt.LoginGate or {
    Enabled = true,
    Title = "Redeem Elternative Library Key",
    DiscordURL = nil,            -- e.g. "https://discord.gg/xxxx"
    PurchaseURL = nil,           -- optional
  ...
  local function Notify(cfg)
    ...
    task.delay(dur,function() tplay(N,.1,{GroupTransparency=1}); task.delay(.1,function() N:Destroy() end) end)
  end

  -- Window (center, small/clean)
  ...
  -- buttery drag with inertia (no teleport)
  do
    ...
    UIS.InputChanged:Connect(function(i)
      ...
      Root.Position = UDim2.fromOffset(newPos.X + Root.Size.X.Offset*0, newPos.Y + Root.Size.Y.Offset*0)
  ...
  -- Tabs/Sections and components (Late-like API)
  ...
  -- shared row helpers
  local function row(parent,h) local R=Instance.new("Frame"); R.BackgroundColor3=Theme.Panel; R.Size=UDim2.new(1,0,0,h); R.Parent=parent; rc(R,8); st(R,1,Theme.Outline,.5); return R end
  ...
  function API:AddInput(s)
    ...
    Box.FocusLost:Connect(function(enter) if enter and s.Callback then pcall(function() s.Callback(Box.Text) end) end end)
    return R
  end
  ...
  function API:AddDropdown(s)
    ...
    local function openP()
      ...
      Pop=Instance.new("CanvasGroup"); Pop.GroupTransparency=1; Pop.BackgroundColor3=Theme.Secondary; Pop.Size=UDim2.fromOffset(220,196)
  ...
  -- -------- Login Modal (pixel-like to screenshot) ----------
  local function showGate()
    ...
    Card.Parent = Modal
    rc(Card,14) st(Card,1,Theme.Outline,.4)
    ...
    H1.Text = LOGIN.Title or "Redeem Elternative Library Key"
    H1.Position=UDim2.fromOffset(22,18)
    H1.Size=UDim2.new(1,-44,0,32)
    H1.Parent=Card

    -- link row "click here" / "Click to purchase"
    ...
    Field.Parent = Row
    rc(Field,12) st(Field,1,Theme.Outline,.45)

    local Eye = Instance.new("TextButton")
    Eye.AutoButtonColor=false
    Eye.Text="👁"; Eye.TextSize=18; Eye.Font=Enum.Font.Gotham
    Eye.TextColor3=Theme.Text
    Eye.BackgroundTransparency=1
    Eye.Size=UDim2.fromOffset(34,34)
    Eye.Position=UDim2.fromOffset(10,15)
    Eye.Parent=Field
    ...
    Box.Text=""
    Box.Position=UDim2.fromOffset(46,0)
    ...
    -- Discord card (prominent)
    local Discord = Instance.new("TextButton")
    Discord.AutoButtonColor=false
    Discord.BackgroundColor3 = Theme.Accent2
    ...
    DiscTitle.Position=UDim2.fromOffset(14,7)
    DiscTitle.Size=UDim2.new(1,-20,0,22)
    ...
    DiscSub.Position=UDim2.fromOffset(14,30)
    DiscSub.Size=UDim2.new(1,-20,0,20)
    DiscSub.Parent=Discord

    -- subtle bottom gradient edge (for the blue card shine)
    local DG = Instance.new("UIGradient")
    DG.Color = ColorSequence.new(Color3.fromRGB(88,165,255), Color3.fromRGB(30,115,255))
    DG.Rotation = 0
    DG.Parent = Discord
    ...
    -- RichText link hit-test: we approximate with transparent buttons on the two lines
    local LinkCopy = Instance.new("TextButton")
    LinkCopy.AutoButtonColor=false
    LinkCopy.BackgroundTransparency=1
    LinkCopy.Text = ""
    LinkCopy.Size = UDim2.fromOffset(130,18)
    LinkCopy.Position = UDim2.fromOffset(260,58) -- tuned to sit on "click here"
    LinkCopy.Parent = Card

    local LinkBuy = Instance.new("TextButton")
    LinkBuy.AutoButtonColor=false
    LinkBuy.BackgroundTransparency=1
    LinkBuy.Text = ""
    LinkBuy.Size = UDim2.fromOffset(150,18)
    LinkBuy.Position = UDim2.fromOffset(310,82) -- "Click to purchase"
    LinkBuy.Parent = Card

    -- behavior
    local hidden, realBuffer = true, ""
    Eye.MouseButton1Click:Connect(function()
      hidden = not hidden
      if hidden then
        -- restore bullets
        Box.Text = string.rep("•", #realBuffer)
      else
        Box.Text = realBuffer
      end
    end)
    ...
    Box:GetPropertyChangedSignal("Text"):Connect(function()
      if hidden then
        -- maintain buffer while showing bullets
        local count = utf8.len(Box.Text) or #Box.Text
        -- naive sync: update buffer length only on append, else keep
        if count >= #realBuffer then
          -- can't know exact chars; keep last buffer
        else
          realBuffer = string.sub(realBuffer, 1, count)
        end
        Box.Text = string.rep("•", #realBuffer)
      else
        realBuffer = Box.Text
    ...
    local function doSubmit()
      local key = hidden and realBuffer or Box.Text
    ...
    CloseX.MouseButton1Click:Connect(function() -- allow cancel
      tplay(Modal,.12,{GroupTransparency=1}); tplay(Shade,.10,{GroupTransparency=1})
    ...
    LinkCopy.MouseButton1Click:Connect(function()
      local text = (hidden and realBuffer or Box.Text)
      if setclipboard then setclipboard(tostring(text or "")) end
