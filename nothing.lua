...
local Library = {
	...
	Themes = {
		Default = {
			...
			Accent = Color3.fromRGB(12, 144, 164) -- New accent color
		}
	},
	...
	UITransparency = false -- New property for UI transparency
}
...
-- Function to apply transparency and glass effect
function Library:SetUITransparency(enabled)
	Library.UITransparency = enabled
	local transparencyValue = enabled and 0.1 or 0
		...
		if obj:IsA("Frame") or obj:IsA("TextButton") then
			if obj.BackgroundTransparency < 1 then
				TweenService:Create(obj, TweenInfo.new(0.3), {
					BackgroundTransparency = obj.BackgroundTransparency + transparencyValue
				}):Play()
			end
		end
	end
	...
	if enabled then
		if not Container:FindFirstChild("BlurEffect") then
...
local function LoadCfg(Config)
	...
	table.foreach(Data, function(a,b)
		if Library.Flags[a] then
			...
			warn("EcstasyUI Library Config Loader - Could not find ", a ,b)
		end
	end)
end
...
function Library:MakeWindow(WindowConfig)
	...
	WindowConfig.Name = WindowConfig.Name or "EcstasyUI"
	...
	WindowConfig.IntroText = WindowConfig.IntroText or "Starting EcstasyUI"
	...
	local WindowStuff = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 10), {
		...
		SetChildren(SetProps(MakeElement("TFrame"), {
			...
			AddThemeObject(SetProps(MakeElement("Label", "EcstasyUI", 12), {
	...
	AddConnection(CloseBtn.MouseButton1Up, function()
		...
		Library:MakeNotification({
			...
			Content = UserInputService.TouchEnabled and "Tap the button or press the keybind to reopen the interface" or "Press the keybind to reopen the interface",
			Time = 5
		})
		WindowConfig.CloseCallback()
	end)

	-- Updated keybind system - now includes Insert key
	AddConnection(UserInputService.InputBegan, function(Input)
		if (Input.KeyCode == Enum.KeyCode.Insert or Input.KeyCode == Enum.KeyCode.LeftControl) and UIHidden == true then
			MainWindow.Visible = true
			MobileReopenButton.Visible = false
			UIHidden = false
		elseif (Input.KeyCode == Enum.KeyCode.Insert or Input.KeyCode == Enum.KeyCode.LeftControl) and UIHidden == false then
			MainWindow.Visible = false
			if UserInputService.TouchEnabled then
				MobileReopenButton.Visible = true
			end
	...
	-- Create Library tab automatically
	...
	-- Add transparency toggle to Library tab
	LibraryTab:AddToggle({
		Name = "Activate Transparency",
		Default = false,
	...
	function TabFunction:MakeTab(TabConfig)
		...
		AddConnection(TabFrame.MouseButton1Click, function()
			...
			-- Updated colors to use the new accent color #0C90A4
			TweenService:Create(TabFrame.Ico, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {ImageTransparency = 0, ImageColor3 = Library.Themes.Default.Accent}):Play()
			TweenService:Create(TabFrame.Title, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {TextTransparency = 0, TextColor3 = Library.Themes.Default.Accent}):Play()
			TabFrame.Title.Font = Enum.Font.GothamBlack
			Container.Visible = true   
		end)
		...
		if TabConfig.PremiumOnly then
			...
			SetChildren(SetProps(MakeElement("TFrame"), {
				...
				AddThemeObject(SetProps(MakeElement("Label", "This part of the script is locked to EcstasyUI Premium users. Purchase Premium in the Discord server.", 12), {
