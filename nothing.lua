--[[
  Ecstays UI • Full Rebuild v7.1
  - Fix: white corner bleed removed (strict clipping), header transparent
  - No password masking; clean redeem flow (OnSubmit -> proceed)
  - No stray "..." dots; submit says "Redeem"
  - Blue Discord button (no icons), strong hover
  - Notifications bottom-right (above blur)
  - Window: zoom open/close + smooth drag w/ inertia
  - Top-right: Minimize + Close only
]]
...
local function rc(p, r) local u=Instance.new("UICorner"); u.CornerRadius=UDim.new(0, r or 14); u.Parent=p; return u end
local function st(p, th, col, tr) local s=Instance.new("UIStroke"); s.Thickness=th or 1; s.Color=col or Color3.fromRGB(70,95,140); s.Transparency=tr or .45; s.ApplyStrokeMode=Enum.ApplyStrokeMode.Border; s.Parent=p; return s end
local function clamp01(x) return x<0 and 0 or (x>1 and 1 or x) end
local function roundAll(root, r)
  for _,v in ipairs(root:GetDescendants()) do
    if (v:IsA("Frame") or v:IsA("TextButton") or v:IsA("ImageLabel") or v:IsA("CanvasGroup")) and not v:FindFirstChildOfClass("UICorner") then
      rc(v, r or 14)
    end
  end
end
...
function Ecstays:CreateWindow(opt)
  ...
  -- window (strict clipping to kill white corner bleed)
  ...
  -- top bar (transparent so corners belong only to Root)
  local Bar = Instance.new("Frame")
  Bar.BackgroundTransparency = 1 -- important: no paint at corners!
  Bar.Size = UDim2.new(1,0,0,44)
  Bar.Parent = Root

  -- inner header fill (no rounding; clipped by Root)
  ...
  -- drag with inertia (no teleport; uses initial offset)
  do
    local DragHandle = Bar -- use the whole top bar area (transparent)
  ...
  function API:AddParagraph(s) local R=row(s.Tab,116); local _,D=labels(R,s.Title,s.Description,0); D.TextWrapped=true; return R end
  ...
  function API:SetTheme(tbl)
    ...
    Sidebar.BackgroundColor3=Theme.Panel
    Main.BackgroundColor3=Theme.Panel
    RootStroke.Color=Theme.Outline
    Title.TextColor3=Theme.Title
  end
  ...
  -- login gate (no masking; blue discord; white corners fixed)
  local function showGate()
    ...
    Submit.Size=UDim2.fromOffset(110,48)
    Submit.Position=UDim2.new(1,-110-12,.5,-24)
    Submit.Parent=Field
    rc(Submit,12)
    ...
    local function doSubmit()
      ...
      if LOGIN.OnSubmit then
        local ok, err = pcall(function() LOGIN.OnSubmit(key, proceed, fail) end)
  ...
  -- expose
  local API = {}
  -- (all API functions are above; expose those table methods)
  for k,v in pairs(getfenv(1)) do end -- noop to keep lint happy
  API.AddTabSection = function(...) return Ecstays._AddTabSection and Ecstays._AddTabSection(...) end
  -- but we kept them local; so return direct closures:
  do
    local env = getfenv(1)
    API.AddTabSection = function(s) return (function() Stored.Sections[s.Name]=s.Order or (#Stored.Sections+1) end)() end
    API.AddTab = function(s)
      if Stored.Tabs[s.Title] then error("[Ecstays] Tab exists: "..s.Title) end
      local btn = (function(n,o) return makeTabBtn(n, o) end)(s.Title, Stored.Sections[s.Section] or 999)
      local page,scroll = makePage(s.Title)
      Stored.Tabs[s.Title] = { Button=btn, Page=page, Scroll=scroll }
      btn.MouseButton1Click:Connect(function() setTab(s.Title) end)
      if not Current then setTab(s.Title) end
    ...
    API.SetTab = function(n) if Stored.Tabs[n] then setTab(n) end end
    API.AddSection = function(s) return (function() local R=row(s.Tab,28); local T=Instance.new("TextLabel"); T.BackgroundTransparency=1; T.Font=Enum.Font.GothamSemibold; T.TextSize=13; T.TextXAlignment=Enum.TextXAlignment.Left; T.TextColor3=Theme.Title; T.Text=s.Name or "Section"; T.Size=UDim2.new(1,-10,1,0); T.Position=UDim2.fromOffset(10,0); T.Parent=R; return R end)() end
    API.AddButton = function(s) local RW=124; local R=row(s.Tab,56); labels(R,s.Title,s.Description,RW); local slot=right(R,RW,30); local B=Instance.new("TextButton"); B.AutoButtonColor=false; B.BackgroundColor3=Theme.Interact; B.Text="Execute"; B.TextColor3=Theme.Title; B.Font=Enum.Font.GothamBold; B.TextSize=13; B.Size=UDim2.fromScale(1,1); B.Parent=slot; rc(B,10); st(B,1,Theme.Outline,.5); B.MouseEnter:Connect(function() tw(B,.08,{BackgroundColor3=Theme.Panel}) end); B.MouseLeave:Connect(function() tw(B,.10,{BackgroundColor3=Theme.Interact}) end); B.MouseButton1Click:Connect(function() if s.Callback then pcall(s.Callback) end end); return R end
    API.AddInput = function(s) local RW=230; local R=row(s.Tab,62); labels(R,s.Title,s.Description,RW); local slot=right(R,RW,30); local Box=Instance.new("TextBox"); Box.ClearTextOnFocus=false; Box.BackgroundColor3=Theme.Interact; Box.TextColor3=Theme.Title; Box.PlaceholderText=s.Placeholder or "type here…"; Box.Font=Enum.Font.Gotham; Box.TextSize=13; Box.Size=UDim2.fromScale(1,1); Box.Parent=slot; rc(Box,10); st(Box,1,Theme.Outline,.5); Box.Focused:Connect(function() tw(Box,.08,{BackgroundColor3=Theme.Panel}) end); Box.FocusLost:Connect(function(enter) tw(Box,.10,{BackgroundColor3=Theme.Interact}); if enter and s.Callback then pcall(function() s.Callback(Box.Text) end) end end); return R end
    API.AddToggle = function(s) local RW=66; local R=row(s.Tab,56); labels(R,s.Title,s.Description,RW); local slot=right(R,RW,28); local Back=Instance.new("Frame"); Back.Size=UDim2.fromScale(1,1); Back.BackgroundColor3=Theme.Interact; Back.Parent=slot; rc(Back,14); st(Back,1,Theme.Outline,.5); local Dot=Instance.new("Frame"); Dot.Size=UDim2.fromOffset(24,24); Dot.Position=UDim2.fromOffset(2,2); Dot.BackgroundColor3=Theme.Secondary; Dot.Parent=Back; rc(Dot,12); local state = s.Default == true; local function set(v) state = v and true or false; if state then tw(Back,.10,{BackgroundColor3=Theme.Accent2}); tw(Dot,.10,{Position=UDim2.fromOffset(RW-2-24,2), BackgroundColor3=Color3.fromRGB(255,255,255)}) else tw(Back,.10,{BackgroundColor3=Theme.Interact}); tw(Dot,.10,{Position=UDim2.fromOffset(2,2), BackgroundColor3=Theme.Secondary}) end end; set(state); R.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then set(not state); if s.Callback then pcall(function() s.Callback(state) end) end end end); return R end
    API.AddKeybind = function(s) local RW=138; local R=row(s.Tab,56); labels(R,s.Title,s.Description,RW); local slot=right(R,RW,30); local Btn=Instance.new("TextButton"); Btn.AutoButtonColor=false; Btn.BackgroundColor3=Theme.Interact; Btn.Text="Set Key"; Btn.TextColor3=Theme.Title; Btn.Font=Enum.Font.GothamBold; Btn.TextSize=13; Btn.Size=UDim2.fromScale(1,1); Btn.Parent=slot; rc(Btn,10); st(Btn,1,Theme.Outline,.5); local cap=false; Btn.MouseButton1Click:Connect(function() if cap then return end; cap=true; Btn.Text="..."; local con; con = UIS.InputBegan:Connect(function(input,gp) if gp then return end; cap=false; local label = input.UserInputType==Enum.UserInputType.Keyboard and tostring(input.KeyCode):gsub("Enum.KeyCode.","") or tostring(input.UserInputType):gsub("Enum.UserInputType.",""); Btn.Text=label; if s.Callback then pcall(function() s.Callback(input) end) end; if con then con:Disconnect() end end) end); return R end
    API.AddDropdown = function(s) return (function() local RW=230; local R=row(s.Tab,64); labels(R,s.Title,s.Description,RW); local slot=right(R,RW,30); local Btn=Instance.new("TextButton"); Btn.AutoButtonColor=false; Btn.BackgroundColor3=Theme.Interact; Btn.Text=""; Btn.Size=UDim2.fromScale(1,1); Btn.Parent=slot; rc(Btn,10); st(Btn,1,Theme.Outline,.5); local TL=Instance.new("TextLabel"); TL.BackgroundTransparency=1; TL.Font=Enum.Font.Gotham; TL.TextXAlignment=Enum.TextXAlignment.Left; TL.TextSize=13; TL.TextColor3=Theme.Title; TL.Text=s.Placeholder or "Select…"; TL.Size=UDim2.new(1,-22,1,0); TL.Position=UDim2.fromOffset(8,0); TL.Parent=Btn; local Arrow=Instance.new("TextLabel"); Arrow.BackgroundTransparency=1; Arrow.Font=Enum.Font.GothamBold; Arrow.TextSize=14; Arrow.TextColor3=Theme.Muted; Arrow.Text="▼"; Arrow.Size=UDim2.fromOffset(18,18); Arrow.Position=UDim2.new(1,-20,0,6); Arrow.Parent=Btn; local Open,Pop=false,nil; local function closeP() if not Pop then return end; tw(Pop,.10,{GroupTransparency=1}); task.delay(.1,function() if Pop then Pop:Destroy(); Pop=nil end end); Open=false end; local function openP() if Open then closeP() return end; Open=true; Pop=Instance.new("CanvasGroup"); Pop.GroupTransparency=1; Pop.BackgroundColor3=Theme.Secondary; Pop.ClipsDescendants=true; Pop.Size=UDim2.fromOffset(240,210); Pop.Position=UDim2.new(0, Btn.AbsolutePosition.X-Root.AbsolutePosition.X, 0, Btn.AbsolutePosition.Y-Root.AbsolutePosition.Y+34); Pop.Parent=Root; rc(Pop,12); st(Pop,1,Theme.Outline,.45); local S=Instance.new("ScrollingFrame"); S.Active=true; S.BorderSizePixel=0; S.BackgroundTransparency=1; S.ScrollBarThickness=4; S.ScrollBarImageTransparency=.1; S.ScrollBarImageColor3=Theme.Interact; S.Size=UDim2.new(1,-10,1,-10); S.Position=UDim2.fromOffset(5,5); S.Parent=Pop; local L=Instance.new("UIListLayout",S); L.Padding=UDim.new(0,6); for k,v in pairs(s.Options or {}) do local O=Instance.new("TextButton"); O.AutoButtonColor=false; O.Size=UDim2.new(1,0,0,28); O.Text=""; O.BackgroundColor3=Theme.Panel; O.Parent=S; rc(O,10); st(O,1,Theme.Outline,.45); local OTL=Instance.new("TextLabel"); OTL.BackgroundTransparency=1; OTL.Font=Enum.Font.Gotham; OTL.TextXAlignment=Enum.TextXAlignment.Left; OTL.TextSize=12; OTL.TextColor3=Theme.Text; OTL.Text=tostring(k); OTL.Size=UDim2.new(1,-10,1,0); OTL.Position=UDim2.fromOffset(6,0); OTL.Parent=O; O.MouseButton1Click:Connect(function() TL.Text=tostring(k); closeP(); if s.Callback then pcall(function() s.Callback(v) end) end end) end; tw(Pop,.12,{GroupTransparency=0}) end; Btn.MouseButton1Click:Connect(openP); UIS.InputBegan:Connect(function(i,gp) if gp or not Open or not Pop then return end; if i.UserInputType==Enum.UserInputType.MouseButton1 then local p=UIS:GetMouseLocation(); local x,y=Pop.AbsolutePosition.X,Pop.AbsolutePosition.Y; local x2,y2=x+Pop.AbsoluteSize.X, y+Pop.AbsoluteSize.Y; if not (p.X>=x and p.X<=x2 and p.Y>=y and p.Y<=y2) then closeP() end end end); return R end)() end
    API.AddSlider = function(s) local RW=116; local R=row(s.Tab,74); labels(R,s.Title,s.Description,RW); local Track=Instance.new("Frame"); Track.BackgroundColor3=Theme.Interact; Track.Size=UDim2.new(1,-(RW+32),0,8); Track.Position=UDim2.new(0,10,1,-18); Track.Parent=R; rc(Track,4); local Fill=Instance.new("Frame"); Fill.BackgroundColor3=Theme.Accent2; Fill.Size=UDim2.fromScale(0,1); Fill.Parent=Track; rc(Fill,4); local Knob=Instance.new("Frame"); Knob.Size=UDim2.fromOffset(16,16); Knob.AnchorPoint=Vector2.new(.5,.5); Knob.Position=UDim2.new(0,0,.5,0); Knob.BackgroundColor3=Color3.fromRGB(255,255,255); Knob.Parent=Track; rc(Knob,8); st(Knob,1,Theme.Outline,.4); local slot=right(R,RW,30); local Box=Instance.new("TextBox"); Box.Size=UDim2.fromScale(1,1); Box.BackgroundColor3=Theme.Interact; Box.TextColor3=Theme.Title; Box.PlaceholderText="0"; Box.Text="0"; Box.Font=Enum.Font.Gotham; Box.TextSize=13; Box.Parent=slot; rc(Box,10); st(Box,1,Theme.Outline,.5); local max=tonumber(s.MaxValue) or 100; local dec=s.AllowDecimals==true; local prec=tonumber(s.DecimalAmount) or 2; local function fmt(n) if dec then local p=10^prec n=math.floor(n*p+0.5)/p return tostring(n) else return tostring(math.floor(n+0.5)) end end; local function setv(n) n=math.clamp(n,0,max); local sc=(max==0) and 0 or (n/max); Fill.Size=UDim2.fromScale(sc,1); Knob.Position=UDim2.new(sc,0,.5,0); Box.Text=fmt(n); if s.Callback then pcall(function() s.Callback(n) end) end end; local dragging=false; local function mval() local m=UIS:GetMouseLocation(); local x0=Track.AbsolutePosition.X; local w=Track.AbsoluteSize.X; return clamp01((m.X-x0)/w)*max end; Track.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then dragging=true; setv(mval()) end end); UIS.InputChanged:Connect(function(i) if dragging and i.UserInputType==Enum.UserInputType.MouseMovement then setv(mval()) end end); UIS.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then dragging=false end end); Box.FocusLost:Connect(function(enter) if enter then setv(tonumber(Box.Text) or 0) end end); setv(tonumber(s.Default) or 0); return R end
    API.AddParagraph = function(s) local R=row(s.Tab,116); local _,D=labels(R,s.Title,s.Description,0); D.TextWrapped=true; return R end
    API.Notify = function(c) Notify(c) end
    API.SetTheme = function(tbl) for k,v in pairs(tbl or {}) do Theme[k]=v end; Root.BackgroundColor3=Theme.Secondary; BarFill.BackgroundColor3=Theme.Primary; Sidebar.BackgroundColor3=Theme.Panel; Main.BackgroundColor3=Theme.Panel; RootStroke.Color=Theme.Outline; Title.TextColor3=Theme.Title end
    API.SetSetting = function(k,v) if k=="Transparency" then Trans=math.clamp(v or Trans,0,.95); Root.GroupTransparency=Trans elseif k=="Size" then Size=v; Root.Size=v elseif k=="Theme" and typeof(v)=="table" then API.SetTheme(v) elseif k=="Keybind" then Key=v end end
    API.Show = function() open() end
    API.Hide = function() Root.Visible=false end
    API.Destroy = function() SG:Destroy(); SG_Notify:Destroy() end
  end
